...
const ChatInterface = ({ appointmentId, chatRoomId, onClose, initialChatRoom, isFloating, onMessagesRead }) => {
  ...
  if (loading) {
    return (
      <div className="chatInterface_loading">
        <div className="chatInterface_spinner"></div>
        <p className="chatInterface_loadingText">Connecting to chat...</p>
      </div>
    );
  }
  ...
  if (!chatRoom && !loading && !chatRoomId && !appointmentId && !initialChatRoom) {
    ...
    return (
      <div className="chatInterface_error">
        <div className="chatInterface_errorIcon">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="12" cy="12" r="10" stroke="#dc2626" strokeWidth="2"/>
            <path d="M15 9L9 15M9 9L15 15" stroke="#dc2626" strokeWidth="2" strokeLinecap="round"/>
          </svg>
        </div>
        <h3 className="chatInterface_errorTitle">Chat Unavailable</h3>
        <p className="chatInterface_errorMessage">Unable to load chat room. Please try again later.</p>
        <button onClick={onClose} className="chatInterface_closeBtn">
          Close
        </button>
      </div>
    );
  ...
  if (chatRoom && !loading && chatRoom._id && !chatRoom.doctor && !chatRoom.patient && !chatRoom.isDirectChat) {
    ...
    return (
      <div className="chatInterface_loading">
        <div className="chatInterface_spinner"></div>
        <p className="chatInterface_loadingText">Loading chat details...</p>
      </div>
    );
  }

  return (
    <div className="chatInterface_container">
      {/* Chat Header - Only show when not in floating mode */}
      {!isFloating && (
        <div className="chatInterface_header">
        <div className="chatInterface_info">
          <h3 className="chatInterface_title">
          ...
          <div className="chatInterface_connectionStatus">
            <span className={`chatInterface_statusIndicator ${isConnected ? 'chatInterface_connected' : 'chatInterface_disconnected'}`}>
      ...
      {/* Messages Container */}
      <div className="chatInterface_messagesContainer">
        {!Array.isArray(messages) || messages.length === 0 ? (
          <div className="chatInterface_noMessages">
            <div className="chatInterface_noMessagesIcon">
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <p className="chatInterface_noMessagesText">No messages yet. Start the conversation!</p>
          </div>
        ) : (
        ...
        {typingUsers.length > 0 && (
          <div className="chatInterface_typingIndicators">
            {typingUsers.map(user => (
              <div key={user.userId} className="chatInterface_typingIndicator">
                <span className="chatInterface_typingUser">{user.userName}</span>
                <span className="chatInterface_typingDots">
                  <span></span>
                  <span></span>
                  <span></span>
                </span>
      ...
      {/* Message Input */}
      <div className="chatInterface_inputContainer">
        <div className="chatInterface_inputWrapper">
          <button
            onClick={() => setShowFileUpload(true)}
            className="chatInterface_fileUploadBtn"
            title="Upload file"
          >
